...
#   plot( pro_WT[,1], pro_WT[,3*i+1], type="l", col = color_WT, ylab = y_lab, ylim = c(0, 1.1*max_y ) , main = label[i], xaxt="n", cex.lab =  1.5, xlab="", cex.axis = 1.5, cex.main = 2)
    ...
    plot(x_data[[1]][,1], x_data[[1]][,3*i+1], type="l", col = cols[1], ylab = y_lab, ylim = c(0, 1.1*max_y ) , main = label[i], xaxt="n", cex.lab =  1.5, xlab="", cex.axis = 1.5, cex.main = 2)
	
	for(j in 2:length){
#	lines(  x_data[[j]][,1], x_data[[j]][,3*i+1],  col = cols[j], pch = j, type = "o", cex = 0.5)
	lines(  x_data[[j]][,1], x_data[[j]][,3*i+1],  col = cols[j], pch = j, type = "l", cex = 0.5)
	}
...
par(xpd=NA)
legend(-160, -3.5,legend= x_lables , col = cols, lwd = 3, bty = "n",  cex = 1.2,  ncol = length) # ncol = length)
