...
plot.window(xlim=c(-extend_length, total_len + gap *4 + extend_length),ylim=c(file_number + 4, 1))
...
par(xpd=NA)
legend(  centromere_pos[1], 6.3 ,legend= legend_txt , col = cols_db, lwd = 4, bty = "n",  cex = 1, ncol = 7
#,   ncol = length
)
dev.off()