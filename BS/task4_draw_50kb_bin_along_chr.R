...
#		   col = c("red3", "red4", "dodgerblue3" , "dodgerblue","chartreuse2", "chartreuse3"),cex = 0.8, lty = 1,bty = "n" , xjust = 1, yjust = 1 , ncol = 3,horiz = F)
	...
	legend(max_len, max + 100, paste(type, c("CG (Watson)","CHG (Watson)", "CHH (Watson)", "CG (Crick)", "CHG (Crick)","CHH (Crick)"),sep = " ") ,
		   col = c("red3", "dodgerblue3" , "chartreuse2", "red4", "dodgerblue", "chartreuse3"),cex = 0.8, lty = 1,bty = "n" , xjust = 1, yjust = 1 , ncol = 2,horiz = F)
...
#		ylab = paste(chr, "Methylcytosines / 50 kb", sep ="\n")
		...
		plot(x[,1],x[,2], type = "l", ylim = c(-max,max),xlim = c(0,maxx), col = cols[2*j], ylab = "Methylcytosines / 50 kb", 
			 xlab = "", axes =F ) #xaxt = "n", bty = "n" (no box) box()
		box (which = "figure")
