# R --slave --vanilla --args  png_pre  indir < /Users/tang58/scripts_all/R_scripts/draw_loci_distribution_in_5_chr/draw_distribution_in_chr_use_coor_format_multiple_input_v0.2.R
...
x_lables = sub("(.+)_vs_.+", "\\1", files, perl =T)
...
#> h <- cbind(h, matrix(values, byrow=TRUE, ncol=2, dimnames=list(NULL, c("num", "prim"))  )  )


for (i in 1:length(files) ){
#	data_individual = x_data[[i]][,1:2];
	data_individual = x_data[[i]][,1:2];# get coor col
	values = unlist(strsplit(as.character(data_individual[,1]), ":"))# split :
	data_individual = cbind(data_individual, matrix(values, byrow=TRUE, ncol=2, dimnames=list(NULL, c("chr", "coor2")) ))# 1:2-3 X 1 2-3
	values = unlist(strsplit(as.character(data_individual[,4]), "-"))# split - 1:2-3 X 1 2-3 start end
	data_individual = cbind(data_individual, matrix(as.numeric(values), byrow=TRUE, ncol=2, dimnames=list(NULL, c("start", "end")) ))# 1:2-3 X 1 2-3 2 3

	data_individual[,c(1,2)] = data_individual[,c(3,5)]#  chr start coor2 start end
	data_individual[,2] = as.numeric(data_individual[,2])
#	print(data_individual[1:5,])
...
#	chr = paste("chr", j, sep="");
		...
		data[,4] = data[,3] + 0.08;
		segments(data[,3], -data[,2], data[,4], -data[,2],  lwd = thin_lwd , col = cols_db[i] )
...
#par(xpd=NA)
legend( 0 , -32000000,legend= x_lables , col = cols_db, lwd = 2, bty = "n",  cex = 1.2
,   ncol = length
)
