...
# R --slave --vanilla --args  file_pattern(Gene_all TE_all) png_pre  type_TE_Gene dir < 
#/Users/tang58/scripts_all/R_scripts/Gene_TE_Meth_Level_Distribution/gene_TE_bin_files_R_cmd_for_all_command_tmp_break_axis.r# TE_all; Gene_all
...
#file_pattern ="MBD7_vs_WT_ChIP_combine_Dec15_keep_dup_all_4091peaks"
# png_pre		 = "MBD7_vs_WT_ChIP_combine_Dec15_keep_dup_all_4091peaks"
# type_TE_Gene ="TE"
# indir = "/Users/tang58/misc/Zhaobo_Lang/6_mbd7_91-2/ChIP_peak/meth_level_over_Peak/output"
#
...
x_lables = sub(".+_in_(.+)_binNum10.+.txt", "\\1", files, perl =T)
...
#ylim_val = list( c(75,85), c(37,47), c(0,10), c(24,34)) # for real mbd data
ylim_val = list( c(20,30), c(0,10), c(0,10), c(0,10)) # for real mbd data
...
# max_y = max( pro_WT[, 3*i+1 ],  pro_JKZ3[, 3*i+1],  pro_JKZ4[, 3*i+1],  pro_ros1[, 3*i+1], pro_newJKZ4[, 3*i+1] )
	...
	plot(x_data[[1]][ord,1], x_data[[1]][ord,3*i+1], type="l", 
		 col = cols[1], 
...
if(i == 1){axis.break(2,)}
	...
	par(xpd=F)
	for(j in 2:length){
		lines(  x_data[[j]][ord,1], x_data[[j]][ord,3*i+1],
			  col = cols[j],
			  pch = j, 
			  type = "l", 
			  cex = 0.5 , 
