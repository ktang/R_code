...
my $debug = 0;
my $usage = "$0 <dir> <bed_file> <output>";
die $usage unless(@ARGV == 3);
my ($dir, $bed, $output) = @ARGV[0..2];

if (!(-e "$dir/$bed") or (-e "$dir/$output")){
	die "wrong input or output\n";
}

open (OUT, ">$dir/$output");

open(IN, "$dir/$bed") or die "Can't open $bed:$!";
...
#	next unless(/\w+/);
	...
	$bed_regions{$chr}->{$start} = [$start, $end];
}
...
while(<GFF>){
	...
	if($temp[2] eq "gene" || $temp[2] eq "pseudogene" || $temp[2] eq "transposable_element_gene"){
		foreach my $st(keys %{$bed_regions{$chr}}){
		#	my ($start, $end, $p,$mstart, $mend) = @{$bed_regions{$chr}->{$st}};
			my ($start, $end) = @{$bed_regions{$chr}->{$st}};
...
print OUT join("\t", ("Chr", "Start", "End","Gene", "Strand","GeneType", "GeneAnnot",
                  "TE", "TEFamily", "Intergenic")), "\n";
foreach my $chr(sort keys %bed_regions){
	my %reg = %{$bed_regions{$chr}};
