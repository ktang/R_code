...
#	lines(  x_data[[j]][,1], x_data[[j]][,3*i+1],  col = cols[j], pch = j, type = "o", cex = 0.5)
	...
	axis(1, labels = c("upstream 2kb", "TE", "downstream 2kb"), at = c(11,31,51), tick =F, cex.axis = 1.3)
}

par(xpd=NA)
legend(-160, -15,legend= x_lables , col = cols, lwd = 3, bty = "n",  cex = 1.2,  ncol = length) # ncol = length)
